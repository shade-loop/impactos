...
import DependencyGraph, { type SelectedNode } from "./components/DependencyGraph";
...
function App() {
  ...
  const impactData = useMemo(() => {
    ...
    return getMockImpactData(selected.id, selected.label, selected.type, impactMap);
  }, [selected, impactMap]);
    ...
    <div className="min-h-screen bg-[#080b12] text-slate-100">
      ...
      <header className="border-b border-white/10 bg-[#0b0f17]/80">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold tracking-tight">ImpactOS</h1>
              <p className="text-xs text-slate-500">Developer impact intelligence</p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-7xl px-6 py-10">
        ...
        <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatCard label="Files"     value="247"   description="Analyzed" />
          <StatCard label="Functions" value="1,842" description="Indexed"  />
          <StatCard label="Tests"     value="86"    description="Detected" />
          <StatCard label="Services"  value="12"    description="Mapped"   />
        </section>
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-3">
          ...
          <div className="min-h-[420px] rounded-xl border border-white/10 bg-white/[0.03] lg:col-span-2">
            ...
            <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
              <div>
                <h3 className="font-medium text-white">Dependency Graph</h3>
                <p className="mt-1 text-xs text-slate-500">Repository architecture</p>
              </div>
              <GitBranch className="h-5 w-5 text-slate-500" />
            </div>

            {/* Graph canvas — explicit height so ReactFlow has a bounded container */}
